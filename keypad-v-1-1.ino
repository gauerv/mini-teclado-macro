...
void loop() { 
  ...
  for(uint8_t row = 0; row < ROWS; row++){  // Para cada uma das linhas:
    ...
    for(uint8_t col = 0; col < COLS; col++){    // Para cada coluna:
      if(digitalRead(colPins[col]) == LOW){       // Se estiver conectado ao GND:
        if((pressTime + DEBOUNCE < millis())){      // Caso seja a última tecla tenha sido solta há mais de DEBOUNCE milissegundos:
          sendKey(row, col);                          // Enviar clique.
          while(digitalRead(colPins[col]) == LOW);    // Travar a leitura até que a tecla seja solta.
          pressTime = millis();                       // Atualizar tempo de debounce.
        }
      }
    }
